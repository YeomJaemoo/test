...
def main():
    ...
    st.header("😶죄송하지만 이 첷바드는 참고용으로 사용하세요!", divider='rainbow')
    ...
    with st.sidebar:
        ...
        if st.button("말하기", key="speak_button"):
            with st.spinner("음성을 인식 중..."):
                recognizer = sr.Recognizer()
                try:
                    with sr.Microphone() as source:
                        recognizer.adjust_for_ambient_noise(source)
                        st.info("마이크가 정상적으로 감지되어있습니다. 음성을 말씀하세요.")
                    ...
                    st.error("마이크가 감지되지 않습니다. 마이크가 정상하게 설치되어 있는지 확인하세요.")
